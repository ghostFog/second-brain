...
> 分类：sync | 状态：设计待评审
> 对应需求：`需求-平台规划.md`（GIT-09 ~ GIT-13，待登记）
...
| GIT-13 | 常驻 Git 状态指示 | 底部常驻状态条显示「分支 · 待提交数 · 是否未初始化」，点击可操作；周期+动作后刷新 |
| GIT-14 | 目录树状态着色 | 文件树按 Git 工作区状态着色区分：**新增（未跟踪）/ 加入跟踪（暂存）/ 修改 / 未变动**；含变更的目录给出聚合指示 |
...
### 2.5 常驻 Git 状态指示（GIT-13）

- 插件在布局左下角（Ribbon 之下 / 左下贴边）注入一个常驻 chip，不依赖宿主新增 UI。
- 内容：`分支 · 待提交数`；未初始化显示 `Git · 未初始化`（点击引导 init，同现有 Ribbon）。
- 刷新：进入插件页签、任何 git 动作后、及 30s 轮询（成功才更新，failure 保留上次值并隐藏报错）。若未安装 git 或非仓库，降级隐藏或不显示计数。
- 复用现有 `git status --porcelain` 统计待提交数；拦截多次并发轮询防堆积。
- 位置不遮挡现有底部位图/文本：固定在左侧、Ribbon 列下方，小字号、`--note-*` 令牌配色（见 styles.css）。
...
- 原 `git-log` / `git-revert` 保留（库级）；`git-status`（Ribbon）保持不变。
...
- **状态定义与优先级**（同文件互斥，按序取最高；配色对齐 IntelliJ IDEA 深色模式）：
  ...
  | 修改（工作区改动） | 次列 M / D | 蓝 |
  | 未变动（已跟踪且干净） | ls-files 命中且 status 未出现 | 白（亮色默认） |

- **着色对象**：遍历 `#file-tree [data-path="相对库根"]`，依映射给节点加状态 class（`gs-untracked / gs-staged / gs-modified`；干净不加）；路径大小写终审按库根相对路径归一。
- **目录聚合**：目录节点若子树内含任一非「未变动」文件，加一个细点/小色标指示「本目录有变更」；不做逐级色块，避免花屏。
- **刷新**：与常驻状态条共用同一个状态周期（30s）+ 所有 git 动作后触发；DOM 中文件树为空/正在重渲染时跳过。
...
3. 插件：新增单文件历史/还原 action + 弹框；历史弹框改 `--name-status` 并展示改动文件；还原增加清理复选；注入常驻状态条 + 目录树状态着色（共用 30s 状态周期）。
...
- 交互（右键菜单、状态条、目录树着色）无法自动化回归，标注「待手动验证」。
