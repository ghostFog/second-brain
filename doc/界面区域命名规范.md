...
> **术语约定（2026-09-15，界面命名 2026-09-22 调整为「编辑界面」）**：
> - **编辑界面** = Ribbon 按钮「编辑界面」（原「编辑器」「知识库」）对应的主工作区视图（`views/vault.html`，路由 `#/editor`）。界面名称用「编辑界面」，与「打开项目」工作区（同为该系统编辑器界面）区分同名歧义；模块名仍为「知识库」（ED- 需求模块）。
> - **编辑区域** = 编辑界面视图内的编辑展示区（.view-row 中部，含 Tab 栏 + vditor 编辑区）。
> - **编辑器** = 特指「编辑界面-编辑区域」里的编辑器（vditor 编辑区）。以后提到「编辑器」，默认指 `编辑界面-编辑区域-编辑器`。
