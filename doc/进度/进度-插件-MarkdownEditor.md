...
| MD-05 | 图片/附件上传（原 ED-48） | Markdown Editor 编辑器支持上传「图片+附件」：vditor 工具栏新增 upload 按钮，上传落盘到 `.resources`（UUID 命名保留扩展名），图片插 `![名](url)`、附件插 `> [!attach] 名 url` 卡片引言；`note://` 协议 `vault_res` 路由 → `.resources`；附件点击经捕获拦截调 `notes:openResource` 用系统默认程序打开；`.resources` 在 walkNotes/scanVaultMeta 跳过 | 测试完成 | 2026-09-11 | 2026-09-11 | 见原 `进度-编辑器.md` ED-48 归档记录（已迁出本文件）；交互增强含拖拽上传/右键插入/Obsidian 附件卡片语法；回归 124 通过 |
