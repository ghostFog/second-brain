  ...
  function settingsPanelHtml(cat) {
    ...
    if (cat === 'AI 问答') {
      return settingsPanel(cat, CATS[cat],
        ...
        + '<div class="flex items-center justify-between mt-3">'
        + '<div><div class="text-[14px]" style="color:var(--note-ink);">打印完整提示词</div>'
        + '<div class="text-caption" style="color:var(--note-ink-3);">调试用：开启后每次 AI 问答把完整请求消息（含系统提示词）打印到控制台</div></div>'
        + '<label class="ai-switch shrink-0" title="调试用：把完整提示词打印到控制台"><input type="checkbox" data-ai-cfg="printFullPrompt"><span class="ai-switch-slider"></span></label></div>'
        + '</section>'
