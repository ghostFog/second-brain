  ...
  function settingsPanelHtml(cat) {
    ...
    if (cat === '常规') {
      ...
      return settingsPanel(cat, CATS[cat],
        '<section class="settings-group" id="gen-sec-main">'
        ...
        + '<div class="flex items-center justify-between py-3"><div class="flex-1 pr-4"><div class="text-[14px]" style="color:var(--note-ink);">开发者模式</div><div class="text-caption" style="color:var(--note-ink-3);">开启后 AI 问答显示检索时长/首token/每秒token 与资料相关度，并自动打印完整提示词（调试用）</div></div><label class="toggle"><input type="checkbox" data-skey="devMode"><span class="toggle-track"></span></label></div>'
        + '</section>'
        + '<section class="settings-group" id="gen-theme-mode">'
        ...
        + '</div></section>'
        + '<section class="settings-group" id="gen-accent-color">'
        ...
        + '</div></section>'
        + '<section class="settings-group" id="gen-font-settings">'
        ...
        + '</section>'
        + '<section class="settings-group" id="gen-interface-density">'
        ...
        + '</div></section>'
        + '<section class="settings-group" id="gen-ribbon-navigation-bar">'
        ...
        + '</section>'
        + '<section class="settings-group" id="gen-other">'
        ...
        + '<div class="border-t py-3" style="border-color:var(--note-border);"><div class="text-[14px]" style="color: var(--note-ink);">运行日志目录</div><div class="text-caption mb-2" style="color: var(--note-ink-3);">git 操作失败等运行日志写入位置，留空使用默认目录</div><div class="flex items-center gap-2"><input id="gen-logdir" type="text" placeholder="默认：userData/logs" autocomplete="off" class="flex-1 px-2 py-1.5 rounded text-[13px]" style="color:var(--note-ink);background:var(--note-surface-2);border:1px solid var(--note-border);"><button data-saction="pick-logdir" class="px-3 py-1.5 rounded text-[12px] shrink-0 border" style="color:var(--note-ink-2);background:var(--note-surface-2);border-color:var(--note-border);">浏览…</button><button data-saction="set-logdir" class="px-3 py-1.5 rounded text-[12px] shrink-0" style="background:var(--note-brand-600);color:#FFFFFF;">保存目录</button><button data-saction="view-log" class="px-3 py-1.5 rounded text-[12px] shrink-0 border" style="color:var(--note-ink-2);background:var(--note-surface-2);border-color:var(--note-border);">查看</button></div></div>'
        + '<div class="border-t py-3" style="border-color:var(--note-border);"><div class="text-[14px]" style="color: var(--note-ink);">单日志文件大小(MB)</div><div class="text-caption mb-2" style="color: var(--note-ink-3);">单份日志（app.log / error.log）超限自动轮换拆分（旧文件 → .1），默认 5</div><div class="flex items-center gap-2"><input id="gen-logmax" type="number" min="1" max="1024" step="1" placeholder="5" class="w-24 px-2 py-1.5 rounded text-[13px]" style="color:var(--note-ink);background:var(--note-surface-2);border:1px solid var(--note-border);"><button data-saction="set-logmax" class="px-3 py-1.5 rounded text-[12px] shrink-0" style="background:var(--note-brand-600);color:#FFFFFF;">保存</button></div></div>'
        + '</div>'
        + '</section>');
    }
    if (cat === '编辑器') {
    ...
    if (cat === '隐私与安全') {
      return settingsPanel(cat, CATS[cat],
        ...
        + '</section>'
        // 应用密码（SEC-01/02）：密码本地 scrypt 哈希存储；开启后启动/超时需输入密码
    ...
    if (cat === 'AI 问答') {
      return settingsPanel(cat, CATS[cat],
        ...
        + '<div><div class="text-[14px]" style="color:var(--note-ink);">启动常驻</div>'
        + '<div class="text-caption" style="color:var(--note-ink-3);">应用启动时自动常驻最后使用的问答模型（Ollama keep_alive=-1，加速首次问答）</div></div>'
        + '<label class="ai-switch shrink-0" title="应用启动时自动常驻最后使用的问答模型"><input type="checkbox" data-ai-cfg="autoResidentOnStart" checked><span class="ai-switch-slider"></span></label></div>'
        + '<div class="flex items-center justify-between mt-3">'
        + '<div><div class="text-[14px]" style="color:var(--note-ink);">携带历史数据</div>'
        + '<div class="text-caption" style="color:var(--note-ink-3);">AI 问答时携带上文对话历史（关闭则每轮只发送当前问题，顶栏开关同步此设置）</div></div>'
        + '<label class="ai-switch shrink-0" title="问答时是否携带上文对话历史"><input type="checkbox" data-ai-cfg="carryHistory" checked><span class="ai-switch-slider"></span></label></div>'
        + '</section>'
        + '<section class="settings-group" id="ai-sec-agent">'
        + '<div class="flex items-center justify-between mb-3"><h3 class="text-body font-semibold" style="color:var(--note-ink);">Agent 配置</h3>'
        + '<button class="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[12px] font-medium" data-ai-saction="agent-add" style="background:var(--note-brand-600); color:#FFFFFF;"><i data-lucide="plus" class="w-3.5 h-3.5"></i>添加 Agent</button>'
        + '</div>'
        + '<div id="ai-agent-list" class="rounded-lg border divide-y" style="border-color:var(--note-border);"></div>'
        + '<p class="text-caption mt-2" style="color:var(--note-ink-3);">Agent 是自定义角色助手：为每个 Agent 配置名称与系统提示词（角色设定/行为指令），问答时作为系统提示词注入。AI 问答页底部可切换使用哪个 Agent；未配置时使用默认助手。</p>'
        + '</section>'
        ...
        + '<div class="flex items-center justify-between py-3 mb-1"><div><div class="text-[14px]" style="color:var(--note-ink);">相关度阈值</div><div class="text-caption" style="color:var(--note-ink-3);">检索相关度低于该值的索引块不作为参考资料（0=不过滤，建议 30~50）</div></div><div class="flex items-center gap-1 shrink-0"><input type="number" min="0" max="100" data-ai-cfg="minSimilarity" class="w-16 px-2 py-1.5 text-[12px] nums text-center rounded-md outline-none" style="background:var(--note-surface-2); border:1px solid var(--note-border); color:var(--note-ink);"><span class="text-caption" style="color:var(--note-ink-3);">%</span></div></div>'
        ...
        + '<div><div class="text-caption mb-1" style="color:var(--note-ink-3);">切分策略</div><select data-ai-cfg="chunkStrategy" class="w-full rounded-md px-2 py-2 text-[13px] outline-none" title="语义分块按标题/段落/句末切分并注入文件名与标题上下文" style="background:var(--note-surface-2); border:1px solid var(--note-border); color:var(--note-ink);"><option value="fixed">固定字符</option><option value="semantic">语义分块</option></select></div>'
        + '<div><div class="text-caption mb-1" style="color:var(--note-ink-3);">单块长度上限（字符）</div><input type="number" min="0" data-ai-cfg="maxChunkSize" class="w-full rounded-md px-3 py-2 text-[13px] nums outline-none" placeholder="300" title="全局统一；0 或留空表示不限制；>0 时每块的宽度不超过块大小与该值的较小者，避免单块过长；语义分块下同样生效" style="background:var(--note-surface-2); border:1px solid var(--note-border); color:var(--note-ink);"></div>'
        + '</div></div>'
        + '<p class="text-caption mt-2" style="color:var(--note-ink-3);">「语义分块」按标题 / 段落 / 句末切分并注入文件名与标题上下文，忽略「相邻重叠」；单块长度上限仍生效。</p>'
